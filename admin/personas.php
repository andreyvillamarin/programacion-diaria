...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_person' && !empty($_POST['nombre_completo']) && !empty($_POST['id_area'])) {
        $stmt = $pdo->prepare("INSERT INTO personas (nombre_completo, id_area, zona) VALUES (?, ?, ?)");
        $stmt->execute([$_POST['nombre_completo'], $_POST['id_area'], $_POST['zona']]);
    }
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="data-table" id="personas-table">
                <thead><tr><th>Nombre Completo</th><th>Área</th><th>Zona</th><th>Estado</th><th>Acciones</th></tr></thead>
                <tbody>
                    <?php foreach ($personas as $persona): ?>
                    <tr>
                        <td><?= htmlspecialchars($persona['nombre_completo']) ?></td>
                        <td><?= htmlspecialchars($persona['nombre_area']) ?></td>
                        <td><?= htmlspecialchars($persona['zona']) ?></td>
