...
if ($action === 'submit_form' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Determine if we are getting a single date or multiple dates
    $fechas_programacion_str = $_POST['fechas_programacion'] ?? null;
    $fechas = [];
    if ($fechas_programacion_str) {
        $fechas = array_map('trim', explode(',', $fechas_programacion_str));
    } else {
        $fechas[] = $_POST['fecha_programacion'];
    }

    if (empty($fechas) || !$area_id) {
        $response['message'] = 'Faltan datos del solicitante (fecha o área). Por favor complete el formulario.';
    ...
    try {
        ...
        foreach ($fechas as $fecha) {
            ...
            $daily_programacion_cards = '';

            if (trim(mb_strtolower($area_nombre, 'UTF-8')) == 'otras áreas') {
                $details = $_POST['other'];
                if (empty($details['nombre_manual']) || empty($details['area_wbe']) || empty($details['actividad']) || empty($details['id_sede'])) {
                     throw new Exception("Faltan datos en el formulario de Otras Áreas para la fecha {$fecha}.");
                }
                ...
                $stmt_detail->execute([':prog_id' => $prog_id, ':sede_id' => $details['id_sede'], ':desayuno' => isset($details['desayuno']) ? 1 : 0, ':almuerzo' => isset($details['almuerzo']) ? 1 : 0, ':comida' => isset($details['comida']) ? 1 : 0, ':ref1' => isset($details['refrigerio_tipo1']) ? 1 : 0, ':ref_cap' => isset($details['refrigerio_capacitacion']) ? 1 : 0, ':transporte' => $details['transporte_tipo'] ?? 'No requiere', ':nombre' => $details['nombre_manual'], ':area' => $details['area_wbe'], ':actividad' => $details['actividad']]);
                
                // For email confirmation
                $daily_programacion_cards .= "<div class='card'><h3>{$details['nombre_manual']}</h3><ul>";
                $daily_programacion_cards .= "<li><strong>Área | WBE:</strong> {$details['area_wbe']}</li>";
                $daily_programacion_cards .= "<li><strong>Actividad:</strong> {$details['actividad']}</li>";
                $daily_programacion_cards .= "</ul></div>";

            } else {
                $people_data = $_POST['people'] ?? [];
                if (empty($people_data)) { throw new Exception("No se seleccionó ninguna persona para programar para la fecha {$fecha}."); }
                ...
                foreach ($people_data as $person_id => $details) {
                    ...
                    if ($has_food || $has_transport) {
                         if (empty($details['id_sede'])) {
                            ...
                            throw new Exception("Se debe seleccionar una sede de destino para {$person_name} para la fecha {$fecha}.");
                        }
                        ...
                        $daily_programacion_cards .= "<div class='card'><h3>{$person_name}</h3><ul>";
                        if (!empty($sede_name)) $daily_programacion_cards .= "<li>✔️ Sede: {$sede_name}</li>";
                        if (isset($details['desayuno'])) $daily_programacion_cards .= "<li>✔️ Desayuno</li>";
                        if (isset($details['almuerzo'])) $daily_programacion_cards .= "<li>✔️ Almuerzo</li>";
                        if (isset($details['comida'])) $daily_programacion_cards .= "<li>✔️ Comida</li>";
                        if (isset($details['refrigerio_tipo1'])) $daily_programacion_cards .= "<li>✔️ Refrigerio Tipo 1</li>";
                        if (isset($details['refrigerio_capacitacion'])) $daily_programacion_cards .= "<li>✔️ Refrigerio Capacitación</li>";
                        if (!empty($details['transporte_tipo'])) $daily_programacion_cards .= "<li>✔️ Transporte: {$details['transporte_tipo']}</li>";
                        $daily_programacion_cards .= "</ul></div>";
                    }
                }
            }
            
            if (!empty($daily_programacion_cards)) {
                $all_programacion_cards .= "<h2>Programación para el {$fecha}</h2>" . $daily_programacion_cards;
        ...
        if ($email && !empty($all_programacion_cards)) {
            $date_str = implode(', ', $processed_dates);
            $html_body = file_get_contents('../templates/email/confirmacion_usuario.html');
            $html_body = str_replace(['{{fecha}}', '{{programacion_cards}}'], [$date_str, $all_programacion_cards], $html_body);
            send_brevo_email([['email' => $email]], "Confirmación de Programación para: {$date_str}", $html_body, $pdo);
        }

        $message = '¡Programación enviada con éxito!';
        if (count($fechas) > 1) {
            $message = '¡Programación para los días ' . implode(', ', $processed_dates) . ' enviada con éxito!';
        }
...
if (isset($_SESSION['user_rol'])) {
    ...
    if ($action === 'download_casino_pdf' && isset($_GET['date'])) {
        ...
        try {
            $date = $_GET['date'];
            // El admin puede especificar una sede en la URL, si no, se usa la del usuario en sesión
